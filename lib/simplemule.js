...
function Flow(parent, ifasyncfn) {
    var steps = [];
    var inputs = [];
    var branches = { };
    var async = false;
    ...
    this.post = function (payload) {
        if (async) {            process(payload, function (err, payload) { });            return;        }                processSync(payload);
    }
    ...
    this.branch = function (name) {
        var branch = new Flow(this, function() {
            async = true;
            var laststep = steps[steps.length - 1];
            
            if (!laststep || !laststep.fn)
                return;
                
            steps[steps.length - 1] = function (payload, cb) {
                var route = laststep.fn(payload);
                var branch = branches[route];
                branch.send(payload, cb);
            }
        });
